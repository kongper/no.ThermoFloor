...
const {
  Mode2Setpoint, Setpoint2Setting, Mode2Number,
} = require('./Z-TRM/ThermostatDeviceSingleMode_mappings.js');
const deviceCapabilityMultiChannelNodeIdObj = require('./deviceCapabilityMultiChannelNodes.js');

class ThermostatDeviceSingleMode extends ZwaveDevice {

  async onMeshInit() {
    const capabilityMultiChannelNodeIdObj = deviceCapabilityMultiChannelNodeIdObj[this.getDriver().id];
    ...
    this.registerCapability('thermostat_mode_single', 'THERMOSTAT_MODE', {
      ...
      multiChannelNodeId: capabilityMultiChannelNodeIdObj['thermostat_state'],
    });
    ...
    this.registerCapability('target_temperature', 'THERMOSTAT_SETPOINT', {
      ...
      multiChannelNodeId: capabilityMultiChannelNodeIdObj['thermostat_mode_single'],
    });
    ...
    if (this.hasCapability('locked')) {
      this.registerCapability('locked', 'PROTECTION', {
        ...
        multiChannelNodeId: capabilityMultiChannelNodeIdObj['locked'],
      });
    }

    // register measure_temperature capabilities if available
    Object.keys(capabilityMultiChannelNodeIdObj).forEach(capabilityId => {
      if (capabilityId.includes('measure_temperature')) {
        ...
        if (this.hasCapability(capabilityId) && subName === undefined) {
          this.log('Registering Capability', capabilityId, 'at multiChannelNodeId', capabilityMultiChannelNodeIdObj[capabilityId]);
          // registerCapability for the internal temperature sensor
          this.registerCapability(capabilityId, 'SENSOR_MULTILEVEL', {
            getOpts: { getOnStart: true },
            multiChannelNodeId: capabilityMultiChannelNodeIdObj[capabilityId],
          });
            ...
            reportParser: report => {
              if (report
                ...
                if (report.Level.Scale === 0) {
                  if (this.getSetting('Temperature_thermostat') === subName) this.setCapabilityValue('measure_temperature', report['Sensor Value (Parsed)']);
                  return report['Sensor Value (Parsed)'];
                }
                if (report.Level.Scale === 1) {
                  if (this.getSetting('Temperature_thermostat') === subName) this.setCapabilityValue('measure_temperature', (report['Sensor Value (Parsed)'] - 32) / 1.8);
    ...
    if (this.hasCapability('measure_humidity')) {
      this.registerCapability('measure_humidity', 'SENSOR_MULTILEVEL',
        {
          getOpts: { getOnStart: true },
          multiChannelNodeId: capabilityMultiChannelNodeIdObj['measure_humidity'],
        });
    }
    ...
    if (this.hasCapability('measure_battery')) {
      this.registerCapability('measure_battery', 'BATTERY',
        {
          getOpts: { getOnStart: true },
          multiChannelNodeId: capabilityMultiChannelNodeIdObj['measure_battery'],
        });
    }

    if (this.hasCapability('meter_power')) this.registerCapability('meter_power', 'METER', { multiChannelNodeId: capabilityMultiChannelNodeIdObj['meter_power'] });
    if (this.hasCapability('measure_power')) this.registerCapability('measure_power', 'METER', { multiChannelNodeId: capabilityMultiChannelNodeIdObj['measure_power'] });
    if (this.hasCapability('measure_voltage')) this.registerCapability('measure_voltage', 'METER', { multiChannelNodeId: capabilityMultiChannelNodeIdObj['measure_voltage'] });
    ...
    if (this.hasCapability('button.reset_meter')) {
      ...
      this.registerCapabilityListener('button.reset_meter', async () => {
        ...
        if (typeof this.meterReset === 'function') return this.meterReset(capabilityMultiChannelNodeIdObj['button.reset_meter']);
